...
int recvRetCommandFinished(void)
{
	...
	while(HAL_UART_GetState(&huart3) != HAL_UART_STATE_READY && tries < 50){
		DWT_Delay_us(1000);
		tries++;
	}

	if (UART_RX[0] == NEX_RET_CMD_FINISHED && UART_RX[1]==0xFF && UART_RX[2]==0xFF && UART_RX[3]==0xFF)
		ret = 1;
...
int recvRetNumber(void)
{
	...
	while(HAL_UART_GetState(&huart3) != HAL_UART_STATE_READY ){
		if (UART_RX[0] == NEX_RET_NUMBER_HEAD && UART_RX[5]==0xFF && UART_RX[6]==0xFF && UART_RX[7]==0xFF )
		{
			number= ((uint32_t)UART_RX[4]<<24)|((uint32_t)UART_RX[3]<<16)|(UART_RX[2]<<8)|(UART_RX[1]);
		}
	}
...
int NexScrollingTextSetText(int ScrollingText,const char *buffer)
{
	char cmd[50]={0}, buff[50]={0};
	for (int i=0; i<20; i++) buff[i]=buffer[i];
	sprintf (cmd, "g%d.txt=\"%s\"",  ScrollingText,  buff);
	sendCommand (cmd);
	return recvRetCommandFinished ();
}

int NexScrollingTextSetPic(int ScrollingText,int value)
{
	char cmd[10]={0};
	sprintf (cmd, "g%d.pic=%d", ScrollingText, value) ;
	sendCommand (cmd);
	return recvRetCommandFinished ();
}
